# Backlog de Contenido — Enciclopedia Saint Seiya

> Reglas de uso completas: ver `CLAUDE.md` y `.claude/skills/content-loop.md`.
> Cada ítem marcado `[x]` debe tener su archivo `.mdx` correspondiente creado/actualizado en `src/content/`.
> No reordenar ni borrar ítems completados; solo marcar el checkbox y añadir la fecha (`YYYY-MM-DD`).
> Tomar los ítems en orden, de arriba hacia abajo, respetando las prioridades.

## Estado global
- Última iteración: 2026-07-01 (loop, Prioridad 1 — Saga de Hades: Santuario)
...
- [ ] Saga / Ares (`personajes/saga-ares.mdx`) — nota: crear como personaje aparte del ítem de Prioridad 3 (Saga de Géminis) si se decide separar la faceta villano, o fusionar en una sola ficha; evaluar al redactar.
